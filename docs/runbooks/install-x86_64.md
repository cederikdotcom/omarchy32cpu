...
- **The image picker renders real thumbnails** of `.webp` files, both from the libvips JPEG cache and - with `vipsthumbnail` removed and the cache empty, which is `list.sh`'s documented fallback to the source file - straight from the `.webp` through Qt's own plugin. Picking one applies it: driven by key events alone (`Super+Space`, type `background`, `Enter`, `Right`, `Enter`) the wallpaper moved from `0-winding-road.webp` to `1-quattro.webp` and redrew. This needed a fix first; see the next bullet.
...
- **Nine boots, nine first-attempt logins.** Every one reached `pam_unix(greetd:session): session opened for user`, with both `Hyprland` and `quickshell` alive, no crash line under the `omarchy-session` journal tag and no failed unit. The known ~1-in-5 compositor login crash did not recur once on x86_64 in this run - it remains an i686 finding until someone reproduces it here.
