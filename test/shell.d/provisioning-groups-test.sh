...
# A deferred-provisioning install records neither privileged group. Upstream also
# runs install/config/docker.sh here; this fork ships no Docker, so the only
# remaining default-install step that could grant a group is the browser policy.
