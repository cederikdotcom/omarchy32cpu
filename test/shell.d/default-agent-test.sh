...
# Upstream sources install/user/mise.sh here to prove the same stubs are written
# at install time. This fork does not ship mise (mise-bin is in neither official
# Arch nor archlinux32), so there is no install-time stub step to assert; the
# migrations below still have to write working stubs for anyone who installs an
# agent by hand.
