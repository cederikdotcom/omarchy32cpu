...
if pkgs_root is None:
  # FORK: upstream always has the omarchy-pkgs repo beside this one, so upstream
  # fails here. This fork has no package repo at all yet (see "Fork package repo
  # does not exist yet" in docs/RELEASE-NOTES.md) -- there is nothing on any
  # machine for this to find, so a hard failure would be permanently red and would
  # say nothing about the tree. Skip instead, and keep the coverage check live for
  # whoever does have a checkout: point OMARCHY_PKGS_PATH at it, and make this a
  # failure again once the fork ships PKGBUILDs of its own.
  print("ok - PKGBUILD coverage skipped (no omarchy-pkgs checkout; set OMARCHY_PKGS_PATH)")
  sys.exit(0)
