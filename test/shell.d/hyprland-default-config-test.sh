...
# Upstream's last block asserted the same preinstall-marker ordering inside
# bin/omarchy-upgrade-to-quattro. That command is the Omarchy 3 -> 4 upgrade
# path for existing x86_64 installs, which this fork does not ship, so the
# assertion has nothing to read here.
