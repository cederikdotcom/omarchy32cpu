...
# Upstream sources install/user/mise.sh here, exactly as run_logged does, to
# prove the leaf still returns zero. This fork ships no mise (mise-bin is in
# neither official Arch nor archlinux32), so that leaf does not exist and no
# per-user setup step calls the Hermes installer. The guarantee the assertion
# above rests on -- that a caller which cannot tolerate a non-zero exit has to
# say so -- is kept by checking that nothing in install/ invokes the installer
# without `|| true`.
while read -r caller; do
  grep -q 'omarchy-install-hermes-cli || true' "$caller" ||
    fail "an install leaf calling the Hermes installer tolerates its failure" "$caller"
done < <(grep -rl 'omarchy-install-hermes-cli' "$ROOT/install" 2>/dev/null || true)
pass "no install leaf aborts a user setup on a Hermes install that cannot finish"
