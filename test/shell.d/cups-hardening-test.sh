...
# Upstream asserts here that CUPS, its filters, Print Settings and the polkit
# helper all stay in the base package set, so that removing cups-browsed does
# not amount to removing printing. This fork installs no printing stack at all:
# every one of those packages exists on both official Arch x86_64 and
# archlinux32 i686, but a print daemon is not part of a 2 GB, no-GPU core, so
# they are deferred rather than excluded (docs/RELEASE-NOTES.md, "Package
# divergence from upstream"). What still has to hold is that nothing here half
# installs printing: no cups package in the list, and no printing service
# enabled by a fresh install.
for package in cups cups-filters cups-pk-helper system-config-printer cups-pdf cups-browsed; do
  ! grep -qxF "$package" "$packages" ||
    fail "this fork installs no printing stack" "$package is in the base package set"
done
! grep -qE '^[[:space:]]*systemctl[[:space:]]+(enable|start)[[:space:]]+cups' "$ROOT/install/config/enable-services.sh" ||
  fail "a fresh install does not enable a printing service it does not install"

pass "the base install ships no printing stack, and enables no printing service"
