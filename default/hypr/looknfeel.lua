...
hl.config({
  ...
  -- FORK: flat mode. The pixman (CPU) renderer draws no animations, blur,
  -- shadows or rounded corners, so leaving them on only costs layout work for
  -- frames that never show the effect. rounding, shadow and blur above are
  -- already off upstream; animations are the one value this fork flips.
  animations = {
    enabled = false,
  },
})
