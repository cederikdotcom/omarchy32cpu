...
Install `socat` with `omarchy-pkg-add socat` on existing lite systems (it is now in the base list). Run `omarchy-launch-screensaver force` in an unlocked desktop and confirm fullscreen animation and clean input dismissal. Foot 1.13 needs `[colors]`, not `[colors-dark]`, in the screensaver config. Missing user artwork falls back to the shipped logo; user customization is not overwritten. Do not use `omarchy-reinstall-configs` to repair this.
